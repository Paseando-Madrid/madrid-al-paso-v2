 ...
 * Fixes esMadrid XML where fields are inside nested blocks.
 */
...
function pickUrl(ev){
  // En esMadrid el enlace suele venir como "web"
...
function pickVenueLoose(ev){
  // esMadrid suele tener nombrert (recurso/centro)
...
async function main(){
  ...
  // DEBUG mínimo imprescindible: si es HTML/bloqueo, lo veremos aquí
  ...
  if (list.length === 0) {
    console.log("Root keys sample:", Object.keys(root || {}).slice(0, 40));
  }
  ...
  // 4) fallback (si por lo que sea quedó vacío)
  if (ranked.length === 0) {
    const seen2 = new Set();
