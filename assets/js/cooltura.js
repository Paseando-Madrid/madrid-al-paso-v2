...
if (!modal || !mosaic) {
  ...
  function setOverlayBgFromCard(cardEl){
    ...
    // CSS var para usar en ::before/ghost/etc.
    sheet.style.setProperty('--overlay-bg-url', bg || 'none');

    // Ghost real (si existe)
    if(ghost){
      ghost.style.backgroundImage = bg || 'none';
    }
  }
  ...
  function sizeSheetToCanvas(){
    ...
    // Altura “hoja” dentro del canvas (con margen)
    const desiredH = Math.max(320, cRect.height - pad * 2);
    sheet.style.height = `${desiredH}px`;

    // Para evitar que el contenido fuerce altura extra:
    // (CSS terminará el scroll interno con .k-body { overflow:auto } )
  }

  function positionSheetToCard(cardEl){
    if(!sheet || !canvas || !cardEl) return;

    // Asegura altura antes de medir
    sizeSheetToCanvas();
  ...
  function renderMuseos(items){
    ...
    // Si tenemos template, renderizamos “pro” sin string HTML
    if(tplMuseo && 'content' in tplMuseo){
      ...
      for(let i=0; i<max; i++){
        ...
        // mapsUrl: si viene en JSON úsalo; si no, lo derivamos de nombre+dirección
    ...
    // Fallback (si no hay template): render simple con strings
  ...
  async function loadAndRender(key){
    ...
    try{
      ...
      // MUSEOS: modo dedicado (editorial + pin)
      if(cfg.mode === "museos"){
