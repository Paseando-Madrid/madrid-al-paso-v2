/* Cooltura — Mosaic + Overlay anchored (premium)
   ...
   - EXPO: renderer editorial (título bold + sede + sub-sede + artistas + horario + hasta + dirección + pin)
   - EXPO: añade “Sedes recomendadas” manuales (CaixaForum, MAPFRE, Alcalá 31, Casa Encendida)
*/
...
if (!modal || !mosaic) {
  ...
  const EXPO_DECK = "Programación en Madrid · Sedes culturales";
  ...
  const CFG = {
    ...
    // ✅ EXPO deck fijo editorial (no depende del JSON)
    expo:      { title:"Exposiciones de este mes",   deck: EXPO_DECK,                                                          json:"data/agenda-monthly.json",  mode:"group", group:"exhibitions" },
  ...
  const EXPO_VENUES = [
    ...
    { name: "Fundación MAPFRE", address: "Paseo de Recoletos, 23 (y Sala Bárbara de Braganza, 13)" },
  ...
  // Formato:
  // - Título (bold, NO link)
  // - Sede (venue + sub-sede)  -> sub-sede real = space (del JSON)
  // - Artistas (si existe)
  // - Meta: Horario (izq) · hasta X (dcha)
  // - Dirección + pin (derecha)
  function renderExpo(items){
    ...
    kList.innerHTML = list.slice(0, max).map(it => {
      ...
      // ✅ compat: tu JSON trae "space" (subsede visual). Si algún día añades "subvenue", también lo soporta.
      const subvenue = safeText(it.subvenue || it.space);

      const artists  = safeText(it.artists);
      const hours    = safeText(it.hours);
      const until    = safeText(it.dateText);
      ...
      const pin  = maps ? pinLinkHTML(maps, `Ver ubicación de ${venueLine || 'esta exposición'} en Google Maps`) : "";
  ...
  async function loadAndRender(key){
    ...
    kDeck.textContent  = cfg.deck; // ✅ deck base (EXPO ya viene fijo aquí)
    ...
    try{
      ...
      if(cfg.mode === "group"){
        ...
        // ✅ NO usamos g.deck para EXPO (evita la frase del Ayuntamiento)
        if(cfg.group !== "exhibitions" && g?.deck) kDeck.textContent = g.deck;
        if(cfg.group === "exhibitions") kDeck.textContent = EXPO_DECK;
